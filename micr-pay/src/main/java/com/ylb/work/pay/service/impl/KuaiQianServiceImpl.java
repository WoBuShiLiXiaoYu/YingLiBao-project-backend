package com.ylb.work.pay.service.impl;
...
import com.ylb.work.pay.Util.HttpUtil;
import com.ylb.work.pay.Util.Pkipair;
import com.ylb.work.pay.service.KuaiQianService;
...
public class KuaiQianServiceImpl implements KuaiQianService {
    ...
    public void handleQueryOrder() {
        // 从 redis 中获取订单号
        Set<String> orders = stringRedisTemplate.boundZSetOps(RedisKey.KEY_RECHARGE_ORDER_ZSET).range(0, -1);
    ...
    public void kqNotify(HttpServletRequest request) {
        ...
        if (flag) {
            ...
            if ("1001214035601".equals(merchantAcctId)) {
                ...
                System.out.println("订单" + orderId + "，充值结果是: " + rechargeResult);
            } else {
                System.out.println("订单" + orderId + "，充值结果是: 商家号不正确" );
            }
        } else {
        ...
        // 删除 redis 中的订单记录
        stringRedisTemplate.boundZSetOps(RedisKey.KEY_RECHARGE_ORDER_ZSET).remove(orderId);
    }
    ...
    @Override
    public Map<String, String> generateForDate(Integer uid, BigDecimal rechargeMoneyn, String phone) {
        ...
        String orderAmount = rechargeMoneyn.multiply(new BigDecimal("100")).stripTrailingZeros().toPlainString();
    ...
    private String generateOrderId() {
        // id 唯一值
        String key = RedisKey.KEY_RECHARGE_ORDER;
        Long incr = stringRedisTemplate.boundValueOps(key).increment();

        String orderId = DateFormatUtils.format(new Date(), "yyyyMMddHHssSSS") + incr;
        return  orderId;
    }

    @Override
    public void addOrderIdToRedis(String orderId) {
        String key = RedisKey.KEY_RECHARGE_ORDER_ZSET;
    ...
    private void doQuery(String orderId) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(response)) {
                Object detailObject = JSON.parseObject(response).get("orderDetail");
                if (detailObject != null) {
                    // 将查询的 JsonObject 转换为 JsonArray
                    ...
                    if (detailJsonObject != null) {
                        ...
                        System.out.println("处理的订单号: " + orderId + "，结果是: " + result);
                    }
